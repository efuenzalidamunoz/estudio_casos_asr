...
MODEL_DIR = os.path.join(ROOT, "whisper.cpp", "models")
OUTPUT_DIR = os.path.join(ROOT, "scripts_out", "R1")
OUTPUT_CSV = os.path.join(OUTPUT_DIR, "quantization_steps.csv")

MODELS = [
    ("q8_0", os.path.join(MODEL_DIR, "ggml-base-q8_0.bin"), 8),
    ("q5_1", os.path.join(MODEL_DIR, "ggml-base-q5_1.bin"), 7),
    ("q4_0", os.path.join(MODEL_DIR, "ggml-base-q4_0.bin"), 2),
]
...
TYPE_INFO = {
    ...
    # Q4_0:
    # 32 pesos
    # fp16 d + 16 bytes de quants = 18 bytes
    2: ("Q4_0", 32, 18),

    # Q5_0:
    # fp16 d + 4 bytes high bits + 16 bytes quants
    6: ("Q5_0", 32, 22),

    # Q5_1:
    # fp16 d + fp16 m + 4 bytes high bits + 16 bytes quants
    7: ("Q5_1", 32, 24),
...
def fp16_from_bytes(data):
    """
    Python struct 'e' = IEEE-754 half precision.
    """
...
def skip_header(f):
    ...
    # hiperparametros
    # whisper.cpp usa 11 int32 despues del magic
    hparams = [read_i32(f) for _ in range(11)]
    ...
    f.seek(filter_count * 4, os.SEEK_CUR)
...
def analyze_model(path, wanted_type):
    ...
    with open(path, "rb") as f:
        ...
        while True:
            ...
            if len(header) != 12:
                raise RuntimeError(
                    "Archivo terminó en medio de un encabezado de tensor"
                )
            ...
            size = tensor_nbytes(ne, ggml_type)
    ...
    if not all_d:
        raise RuntimeError(
            f"No se encontraron bloques del tipo {wanted_type}"
        )

    return {
        "blocks": len(all_d),
        "tensors": quant_tensor_count,
        "d_mean": sum(all_d) / len(all_d),
        "d_min": min(all_d),
        "d_max": max(all_d),
    }
