...
const WHISPER_CLI = joinpath(@__DIR__, "whisper.cpp", "build-vulkan", "bin", "whisper-cli")
const VULKAN_DEVICE = "8086:9bc4!"  # Intel UHD Graphics (CML GT2); ! oculta los demás dispositivos Vulkan
const LANGUAGE    = "es"
...
function usage_and_exit()
    println(stderr, "uso: julia asr_pipeline_igpu.jl <ruta-al-modelo.bin>")
    println(stderr, "ejemplo: julia asr_pipeline_igpu.jl whisper.cpp/models/ggml-base.bin")
    exit(1)
end
...
function transcribe(wav_file::String, model::String)
    ...
    # Vulkan habilitado: no usar -ng, ya que -ng desactiva la GPU.
    cmd = `$WHISPER_CLI -m $model -f $wav_file -l $LANGUAGE -nt -np -otxt -of $out_prefix -t 4 --temperature 0.0`

    # Fuerza el uso exclusivo de la GPU integrada Intel (8086:9bc4).
    # El signo ! evita que whisper.cpp vea la GTX 1650 y llvmpipe.
    cmd_igpu = addenv(cmd, "MESA_VK_DEVICE_SELECT" => VULKAN_DEVICE)

    elapsed = @elapsed run(pipeline(cmd_igpu; stdout=devnull, stderr=devnull))
